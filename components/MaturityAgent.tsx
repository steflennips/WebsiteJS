...
const MaturityAgent: React.FC = () => {
  ...
  const renderReport = (res: MaturityResult) => (
    <div className="animate-in fade-in zoom-in duration-300 space-y-6 md:space-y-10">
      ...
      <div className="bg-gradient-to-br from-[#00a3ff] to-[#7b2ff7] p-8 md:p-14 rounded-[1.5rem] md:rounded-[3rem] text-white shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center gap-8 md:gap-12 text-center md:text-left">
          ...
          <div>
            ...
            <p className="text-white/80 font-bold uppercase tracking-widest text-[10px] md:text-xs">Data Maturity Niveau</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6 md:gap-10">
        ...
        <div className="glass p-8 md:p-10 rounded-[2rem] border-white/10 space-y-8">
          <div>
            <h5 className="text-indigo-400 font-black uppercase text-[10px] tracking-widest mb-4">Strategische Analyse</h5>
            <p className="text-slate-100 text-lg md:text-xl font-medium leading-relaxed italic">"{res.description}"</p>
          </div>
          
          <div className="space-y-6">
            <h5 className="text-indigo-400 font-black uppercase text-[10px] tracking-widest">Kern Metrics</h5>
            {[
              ...
              <div key={i} className="space-y-2">
                ...
                <div className="h-2 bg-slate-900 rounded-full overflow-hidden">
                  <div 
                    className="h-full bg-gradient-to-r from-[#00a3ff] to-[#7b2ff7] transition-all duration-1000" 
        ...
        <div className="glass p-8 md:p-10 rounded-[2rem] border-white/10 bg-emerald-500/5">
          <h5 className="text-[#4ade80] font-black uppercase text-[10px] tracking-widest mb-6">Directe Actiepunten</h5>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {res.recommendations.map((rec, i) => (
            <div key={i} className="flex gap-4 items-center bg-slate-950/50 p-4 rounded-xl border border-white/5">
              ...
              <span className="text-slate-400 text-xs md:text-sm font-bold uppercase tracking-tight">{rec}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 pt-6">
        ...
        <button 
          ...
          className="px-10 py-5 glass border-white/10 text-white font-bold rounded-2xl hover:bg-white/10 active:scale-95 transition-all text-sm uppercase tracking-widest"
        >
          HERSTART SCAN
        </button>
      </div>
